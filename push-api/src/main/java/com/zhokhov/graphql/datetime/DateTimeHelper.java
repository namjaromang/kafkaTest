...
public class DateTimeHelper {
    ...
    public static final CopyOnWriteArrayList<DateTimeFormatter> DATE_FORMATTERS =
    ...
    public static LocalDate toDate(Timestamp timeStamp) {
    ...
    public static Date createDate(
            int year, int month, int day, int hours, int min, int sec, int millis) {
        long nanos = TimeUnit.MILLISECONDS.toNanos(millis);
        LocalDateTime localDateTime =
                LocalDateTime.of(year, month, day, hours, min, sec, (int) nanos);
        return DateTimeHelper.toDate(localDateTime);
    }

    public static LocalDate localDateAsiaSeoulBeforeDay() {
        return LocalDate.now(ZoneId.of("Asia/Seoul")).minusDays(1);
    }

    public static LocalDateTime localDateTimeAsiaSeoul() {
        return LocalDateTime.now(ZoneId.of("Asia/Seoul"));
    }

    public static LocalTime localTimeAsiaSeoul() {
        return LocalTime.now(ZoneId.of("Asia/Seoul"));
    }

    public static LocalDateTime localDateTimeAsiaSeoulZero() {
        return LocalDateTime.now(ZoneId.of("Asia/Seoul"))
                .withHour(0)
                .withMinute(0)
                .withSecond(0)
